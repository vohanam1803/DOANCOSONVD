function Piano() {
        ...
        function r(i, r, u, f) {
            setTimeout(function() {
                if (piano.stp && !n) piano.stp_cycle || (win_fnc.hider(), select("s1", -1), piano.default(), piano.stp_cycle = !0, piano.chord = [""]);
                    ...
                    piano.deff == u && (n || select("s1", f - 1, f), r[0] == "a" ? piano.music(r, 1) : piano.music(r, 3), piano.play_count++, piano.play_count == piano.nmb - 1 && (n ? t.value = i18n.gettext("â— Record") : (win_fnc.hider(), select("s1", -1)), piano.play_pause = !1, piano.default(), piano.b = !1))
                } catch (i) {}
            }, i)
        }
        var t = $("recplay"),
            i = $("sheet_play");
        if (this.deff++, n) t.value == i18n.gettext("â— Record") ? ($("s1").placeholder = i18n.gettext("Recording..."), t.value = i18n.gettext("â–  Stop"), i.focus(), $("s1").value = "", win_fnc.hider(), this.record = !0) : (t.value = i18n.gettext("â— Record"), $("s1").placeholder = i18n.gettext("Sheet"), retrans.compile(), this.default(), this.record = !1);
        else if (this.record && (retrans.compile(), this.default(), this.record = !1), $("s1").value != "" && (i.value == i18n.gettext("â–Œâ–Œ Pause") ? (i.value = i18n.gettext("â–º Play"), this.play_pause = !0) : (i.value = i18n.gettext("â–Œâ–Œ Pause"), t.value == i18n.gettext("â–  Stop") && (t.value = i18n.gettext("â— Record")), this.play_pause = !1, this.play_cycle = !1, this.stp_cycle = !1, this.stp = !1), $("sheet_stop").style.visibility = "visible", $("sheet_stop_sep").style.visibility = "visible", this.chord.length <= 1 && (this.msx = trans.compile(), this.time = this.msx[0], this.chord = this.msx[1], this.indexes = this.msx[2])), !this.play_pause)
    ...
    this.assist = function() {
        var n;
        if (this.as) {
            for ($("assistd").innerHTML = i18n.gettext("Key assist Off"), n = 0; n < notes.w_n.length; n++) $("a" + notes.w_c[n] + "d").innerHTML = "";
            for (n = 0; n < notes.b_n_.length; n++) $("b" + notes.b_c[n] + "d").innerHTML = "";
            this.as = !1;
            loc.set("assist", "")
        } else {
            for ($("assistd").innerHTML = i18n.gettext("Key assist On"), n = 0; n < notes.w_c.length; n++) $("a" + notes.w_c[n] + "d").innerHTML = "<br/><br/><br/><br/><br/><br/>" + notes.w_n[n];
